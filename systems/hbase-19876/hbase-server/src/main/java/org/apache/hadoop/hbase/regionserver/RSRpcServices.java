...
public class RSRpcServices
    ...
    private class RegionScannerShippedCallBack implements RpcCallback {
        ...
        public RegionScannerShippedCallBack(
            String scannerName,
            Shipper shipper,
            Lease lease
        ) {
        ...
        public void run() {
            for (RegionScanner scanner : scanners) {
                try {
                    ...
                    LOG.error(
                        "Exception while closing the scanner " + scanner,
                        e
                    );
                }
            }
    ...
    private class ScannerListener implements LeaseListener {
        ...
        public void leaseExpired() {
            ...
            if (rsh != null) {
                ...
                HRegion region = null;
                try {
                    region = regionServer.getRegion(
                        s.getRegionInfo().getRegionName()
                    );
    ...
    private static ResultOrException getResultOrException(
        final ClientProtos.Result r,
        final int index
    ) {
        return getResultOrException(
            ResponseConverter.buildActionResult(r),
            index
        );
    }
    ...
    private static ResultOrException getResultOrException(
        ...
        return getResultOrException(
            ResponseConverter.buildActionResult(e),
            index
        );
    }

    private static ResultOrException getResultOrException(
        final ResultOrException.Builder builder,
        final int index
    ) {
     ...
     */
    private boolean startNonceOperation(
        final MutationProto mutation,
        long nonceGroup
    ) throws IOException {
        if (
            regionServer.nonceManager == null || !mutation.hasNonce()
        ) return true;
        ...
        try {
            ...
            throw new InterruptedIOException(
                "Nonce start operation interrupted"
            );
        }
        return canProceed;
    }
     ...
     */
    private void endNonceOperation(
        final MutationProto mutation,
        long nonceGroup,
        boolean success
    ) {
    ...
    private void addResult(
        final MutateResponse.Builder builder,
        final Result result,
     ...
     * @param cellScanner if non-null, the mutation data -- the Cell content.
     */
    private boolean checkAndRowMutate(
        final HRegion region,
        final List<ClientProtos.Action> actions,
        final CellScanner cellScanner,
        byte[] row,
        ...
        for (ClientProtos.Action action : actions) {
            ...
            if (rm == null) {
                rm = new RowMutations(
                    action.getMutation().getRow().toByteArray(),
                    actions.size()
                );
            }
            switch (type) {
                case PUT:
                    Put put = ProtobufUtil.toPut(
                        action.getMutation(),
                        cellScanner
                    );
                    checkCellSizeLimit(region, put);
                    spaceQuotaEnforcement
                        .getPolicyEnforcement(region)
                        .check(put);
                    rm.add(put);
                    break;
                case DELETE:
                    Delete del = ProtobufUtil.toDelete(
                        action.getMutation(),
                        cellScanner
                    );
                    spaceQuotaEnforcement
                ...
                default:
                    throw new DoNotRetryIOException(
                        "Atomic put and/or delete only, not " + type.name()
                    );
            }
        ...
        return region.checkAndRowMutate(
            row,
            family,
            qualifier,
            op,
        ...
        if (r == null) {
            ...
            try {
                ...
                if (canProceed) {
                    ...
                    List<Cell> results = region.get(
                        ProtobufUtil.toGet(mutation, cellScanner),
                        false,
                        nonceGroup,
                        nonce
                    );
        ...
        if (r == null) {
            ...
            try {
                ...
                if (canProceed) {
                    ...
                    List<Cell> results = region.get(
                        ProtobufUtil.toGet(mutation, cells),
                        false,
                        nonceGroup,
                        nonce
                    );
     ...
     */
    private List<CellScannable> doNonAtomicRegionMutation(
        final HRegion region,
        final OperationQuota quota,
        ...
        long nonceGroup,
        final RegionScannersCloseCallBack closeCallBack,
        ...
        long maxQuotaResultSize = Math.min(
        ...
        for (ClientProtos.Action action : actions.getActionList()) {
            ...
            try {
                ...
                if (
                    ...
                    NameBytesPair pair = ResponseConverter.buildException(
                        sizeIOE
                    );
                    resultOrExceptionBuilder.setException(pair);
                    context.incrementResponseExceptionSize(
                        pair.getSerializedSize()
                    );
                    resultOrExceptionBuilder.setIndex(action.getIndex());
                    builder.addResultOrException(
                        resultOrExceptionBuilder.build()
                    );
                    if (cellScanner != null) {
    ...
    private void checkCellSizeLimit(final HRegion r, final Mutation m)
        throws IOException {
        if (r.maxCellSize > 0) {
            CellScanner cells = m.cellScanner();
            while (cells.advance()) {
                int size = PrivateCellUtil.estimatedSerializedSizeOf(
                    cells.current()
                );
    ...
    private void doBatchOp(
        ...
        try {
            ...
            Map<Mutation, ClientProtos.Action> mutationActionMap =
                new HashMap<>();
            int i = 0;
            ...
            OperationStatus[] codes = region.batchMutate(
                mArray,
                atomic,
                HConstants.NO_NONCE,
                HConstants.NO_NONCE
            ...
            for (i = 0; i < codes.length; i++) {
                ...
                ClientProtos.Action currentAction = mutationActionMap.get(
                    currentMutation
                );
                int index = currentAction.hasIndex() || !atomic
                    ? currentAction.getIndex()
                    : i;
                Exception e = null;
                switch (codes[i].getOperationStatusCode()) {
                    case BAD_FAMILY:
                        e = new NoSuchColumnFamilyException(
                            codes[i].getExceptionMsg()
                        );
                        builder.addResultOrException(
                            getResultOrException(e, index)
                        );
                        break;
            ...
            for (Action mutation : mutations) {
                builder.addResultOrException(
                    getResultOrException(ie, mutation.getIndex())
                );
            }
        }
        if (regionServer.metricsRegionServer != null) {
     ...
     * @throws IOException
     */
    private OperationStatus[] doReplayBatchOp(
        final HRegion region,
        ...
        try {
                ...
                if (metaCells != null && !metaCells.isEmpty()) {
                    for (Cell metaCell : metaCells) {
                        CompactionDescriptor compactionDesc =
                            WALEdit.getCompaction(metaCell);
                        boolean isDefaultReplica =
                        ...
                        if (flushDesc != null && !isDefaultReplica) {
                            hRegion.replayWALFlushMarker(
                                flushDesc,
                                replaySeqId
                            );
                            continue;
                        }
                        RegionEventDescriptor regionEvent =
                            WALEdit.getRegionEventDescriptor(metaCell);
                        if (regionEvent != null && !isDefaultReplica) {
                            hRegion.replayWALRegionEventMarker(regionEvent);
                            continue;
                        }
                        BulkLoadDescriptor bulkLoadEvent =
                            WALEdit.getBulkLoadDescriptor(metaCell);
            ...
            return region.batchReplay(
                mutations.toArray(
                    new WALSplitter.MutationReplay[mutations.size()]
                ),
                replaySeqId
            );
        } finally {
    ...
    private static LogDelegate DEFAULT_LOG_DELEGATE = new LogDelegate() {
        ...
        public void logBatchWarning(
            ...
            if (LOG.isWarnEnabled()) {
                LOG.warn(
                    "Large batch operation detected (greater than " +
    ...
    RSRpcServices(HRegionServer rs, LogDelegate ld) throws IOException {
        ...
        try {
            ...
            rpcSchedulerFactory =
                ((RpcSchedulerFactory) rpcSchedulerFactoryClass.newInstance());
        ...
        if (this instanceof MasterRpcServices) {
            ...
            bindAddress = new InetSocketAddress(
                rs.conf.get("hbase.master.ipc.address", hostname),
                port
            );
        } else {
            ...
            initialIsa = new InetSocketAddress(hostname, port);
            bindAddress = new InetSocketAddress(
                rs.conf.get("hbase.regionserver.ipc.address", hostname),
                port
            );
        }
        if (initialIsa.getAddress() == null) {
            throw new IllegalArgumentException(
                "Failed resolve of " + initialIsa
            );
        }
        ...
        ConnectionUtils.setServerSideHConnectionRetriesConfig(
            rs.conf,
            name,
            LOG
        );
        ...
        closedScanners = CacheBuilder.newBuilder()
            .expireAfterAccess(scannerLeaseTimeoutPeriod, TimeUnit.MILLISECONDS)
            .build();
    }
    ...
    protected RpcServerInterface createRpcServer(
        ...
        try {
            return RpcServerFactory.createRpcServer(
                server,
                name,
                getServices(),
                bindAddress, // use final bindAddress for this server.
                conf,
                rpcSchedulerFactory.create(conf, this, server),
                reservoirEnabled
            );
        } catch (BindException be) {
            throw new IOException(
                be.getMessage() +
                    ". To switch ports use the '" +
                    HConstants.REGIONSERVER_PORT +
    ...
    public static String getHostname(Configuration conf, boolean isMaster)
        ...
        if (hostname == null || hostname.isEmpty()) {
            ...
            return Strings.domainNamePointerToHostName(
                DNS.getDefaultHost(
                    conf.get(
                        "hbase." + masterOrRS + ".dns.interface",
                        "default"
                    ),
                    conf.get(
                        "hbase." + masterOrRS + ".dns.nameserver",
                        "default"
                    )
                )
            );
    ...
    public String getScanDetailsWithId(long scannerId) {
        ...
        builder
            .append("table: ")
            .append(scanner.getRegionInfo().getTable().getNameAsString());
        builder
            .append(" region: ")
            .append(scanner.getRegionInfo().getRegionNameAsString());
    ...
    Object addSize(RpcCallContext context, Result r, Object lastBlock) {
        if (context != null && r != null && !r.isEmpty()) {
            for (Cell c : r.rawCells()) {
                context.incrementResponseCellSize(
                    PrivateCellUtil.estimatedSerializedSizeOf(c)
                );
    ...
    private RegionScannerHolder addScanner(
        String scannerName,
        RegionScanner s,
        Shipper shipper,
        HRegion r,
        ...
        Lease lease = regionServer.leases.createLease(
            scannerName,
            this.scannerLeaseTimeoutPeriod,
            new ScannerListener(scannerName)
        );
        RpcCallback shippedCallback = new RegionScannerShippedCallBack(
            scannerName,
            shipper,
            lease
        );
     ...
     */
    @VisibleForTesting
    public HRegion getRegion(final RegionSpecifier regionSpecifier)
        throws IOException {
    ...
    private List<HRegion> getRegions(
        final List<RegionSpecifier> regionSpecifiers,
        final CacheEvictionStatsBuilder stats
    ) {
        List<HRegion> regions = Lists.newArrayListWithCapacity(
            regionSpecifiers.size()
        );
        for (RegionSpecifier regionSpecifier : regionSpecifiers) {
            try {
                regions.add(
                    regionServer.getRegion(
                        regionSpecifier.getValue().toByteArray()
                    )
                );
    ...
    void start() {
        this.scannerIdGenerator = new ScannerIdGenerator(
    ...
    protected void checkOpen() throws IOException {
        if (regionServer.isAborted()) {
            throw new RegionServerAbortedException(
                "Server " + regionServer.serverName + " aborting"
            );
        }
        if (regionServer.isStopped()) {
            throw new RegionServerStoppedException(
                "Server " + regionServer.serverName + " stopping"
            );
        }
    ...
    protected List<BlockingServiceAndInterface> getServices() {
        boolean admin = getConfiguration().getBoolean(
            REGIONSERVER_ADMIN_SERVICE_CONFIG,
            true
        );
        boolean client = getConfiguration().getBoolean(
        ...
        if (client) {
            bssi.add(
                new BlockingServiceAndInterface(
        ...
        return new org.apache.hbase.thirdparty.com.google.common.collect.ImmutableList.Builder<
        ...
        try {
            if (
                ...
                LOG.error(
                    HBaseMarkers.FATAL,
                    "Run out of memory; " +
                        RSRpcServices.class.getSimpleName() +
                        " will abort itself immediately",
                    e
                );
            }
        } finally {
    ...
    public CloseRegionResponse closeRegion(
        ...
        try {
                ...
                if (regionServer.serverName.getStartcode() != serverStartCode) {
                    throw new ServiceException(
                        new DoNotRetryIOException(
                            "This RPC was intended for a " +
                                "different server with startCode: " +
                                serverStartCode +
                                ", this server is: " +
            ...
            final String encodedRegionName = ProtobufUtil.getRegionEncodedName(
                request.getRegion()
            );
            ...
            boolean closed = regionServer.closeRegion(
                encodedRegionName,
                false,
                sn
            );
    ...
    public CompactRegionResponse compactRegion(
        final RpcController controller,
        final CompactRegionRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            requestCount.increment();
            ...
            if (
                ...
                throw new DoNotRetryIOException(
                    "Compactions on this region are " +
                        "disabled due to a space quota violation."
                );
            }
            region.startRegionOperation(Operation.COMPACT_REGION);
            LOG.info(
                "Compacting " + region.getRegionInfo().getRegionNameAsString()
            );
    ...
    @Override
    @QosPriority(priority = HConstants.ADMIN_QOS)
    public FlushRegionResponse flushRegion(
        final RpcController controller,
        final FlushRegionRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            requestCount.increment();
            HRegion region = getRegion(request.getRegion());
            LOG.info(
                "Flushing " + region.getRegionInfo().getRegionNameAsString()
            );
            boolean shouldFlush = true;
            if (request.hasIfOlderThanTs()) {
                shouldFlush =
                    region.getEarliestFlushTimeForAllStores() <
                    request.getIfOlderThanTs();
            }
            FlushRegionResponse.Builder builder =
                FlushRegionResponse.newBuilder();
            if (shouldFlush) {
                boolean writeFlushWalMarker = request.hasWriteFlushWalMarker()
                    ? request.getWriteFlushWalMarker()
                    : false;
                // Go behind the curtain so we can manage writing of the flush WAL marker
                HRegion.FlushResultImpl flushResult = region.flushcache(
                    true,
                    writeFlushWalMarker,
                ...
                if (compactionNeeded) {
                    regionServer.compactSplitThread.requestSystemCompaction(
                        region,
    ...
    public GetOnlineRegionResponse getOnlineRegion(
        final RpcController controller,
        final GetOnlineRegionRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            requestCount.increment();
        ...
        try {
            ...
            if (request.hasCompactionState() && request.getCompactionState()) {
                builder.setCompactionState(
                    ProtobufUtil.createCompactionState(
            ...
            if (
                ...
                builder.setBestSplitRow(
                    UnsafeByteOperations.unsafeWrap(bestSplitRow)
                );
            }
            return builder.build();
        } catch (IOException ie) {
    ...
    public GetRegionLoadResponse getRegionLoad(
        ...
        RegionLoad.Builder regionLoadBuilder =
        ...
        GetRegionLoadResponse.Builder builder =
            GetRegionLoadResponse.newBuilder();
    ...
    public GetServerInfoResponse getServerInfo(
        final RpcController controller,
        final GetServerInfoRequest request
    ) throws ServiceException {
        try {
            checkOpen();
        } catch (IOException ie) {
        ...
        requestCount.increment();
        int infoPort = regionServer.infoServer != null
            ? regionServer.infoServer.getPort()
            : -1;
        return ResponseConverter.buildGetServerInfoResponse(
            regionServer.serverName,
            infoPort
        );
    }
    ...
    @Override
    @QosPriority(priority = HConstants.ADMIN_QOS)
    public GetStoreFileResponse getStoreFile(
        final RpcController controller,
        final GetStoreFileRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            HRegion region = getRegion(request.getRegion());
            ...
            GetStoreFileResponse.Builder builder =
                GetStoreFileResponse.newBuilder();
    ...
    public OpenRegionResponse openRegion(
        ...
        try {
            ...
            int timeout = regionServer.conf.getInt(
                HConstants.HBASE_RPC_TIMEOUT_KEY,
                HConstants.DEFAULT_HBASE_RPC_TIMEOUT
            ) >>
        ...
        long masterSystemTime = request.hasMasterSystemTime()
            ? request.getMasterSystemTime()
            : -1;

        for (RegionOpenInfo regionOpenInfo : request.getOpenInfoList()) {
            final RegionInfo region = ProtobufUtil.toRegionInfo(
                regionOpenInfo.getRegion()
            );
            TableDescriptor htd;
            try {
                String encodedName = region.getEncodedName();
                byte[] encodedNameBytes = region.getEncodedNameAsBytes();
                final HRegion onlineRegion = regionServer.getRegion(
                    encodedName
                );
                ...
                if (Boolean.FALSE.equals(previous)) {
                    ...
                    regionServer.regionsInTransitionInRS.put(
                        encodedNameBytes,
                        Boolean.TRUE
                    );
                }
                ...
                if (previous == null || !previous.booleanValue()) {
                    ...
                    if (htd == null) {
                        htd = regionServer.tableDescriptors.get(
                            region.getTable()
                        );
                    ...
                    if (regionServer.executorService == null) {
                        ...
                        if (region.isMetaRegion()) {
                            ...
                            if (
                                htd.getPriority() >= HConstants.ADMIN_QOS ||
                                region.getTable().isSystemTable()
                            ) {
                                regionServer.executorService.submit(
                                    new OpenPriorityRegionHandler(
                                        regionServer,
                                        regionServer,
                                        region,
                                        htd,
                ...
                LOG.warn(
                    "Failed opening region " + region.getRegionNameAsString(),
                    ie
                );
                if (isBulkAssign) {
     ...
     */
    @Override
    public WarmupRegionResponse warmupRegion(
        final RpcController controller,
        final WarmupRegionRequest request
    ) throws ServiceException {
        final RegionInfo region = ProtobufUtil.toRegionInfo(
            request.getRegionInfo()
        );
        TableDescriptor htd;
        WarmupRegionResponse response =
            WarmupRegionResponse.getDefaultInstance();
        ...
        try {
            ...
            if (onlineRegion != null) {
                LOG.info(
                    "Region already online. Skipping warming up " + region
                );
                return response;
            }
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                    "Warming up Region " + region.getRegionNameAsString()
                );
            }
            ...
            LOG.error(
                "Failed warming up region " + region.getRegionNameAsString(),
                ie
            );
            throw new ServiceException(ie);
        }
    ...
    public ReplicateWALEntryResponse replay(
        ...
        try {
            ...
            boolean isPrimary = RegionReplicaUtil.isDefaultReplica(
                region.getRegionInfo()
            );
            Durability durability = isPrimary
            ...
            for (WALEntry entry : entries) {
                if (!regionName.equals(entry.getKey().getEncodedRegionName())) {
                    throw new NotServingRegionException(
                        "Replay request contains entries from multiple " +
                ...
                if (regionServer.nonceManager != null && isPrimary) {
                    long nonceGroup = entry.getKey().hasNonceGroup()
                        ? entry.getKey().getNonceGroup()
                        : HConstants.NO_NONCE;
                    long nonce = entry.getKey().hasNonce()
                        ? entry.getKey().getNonce()
                        : HConstants.NO_NONCE;
                ...
                Pair<WALKey, WALEdit> walEntry = (coprocessorHost == null)
                    ? null
                    : new Pair<>();
                List<WALSplitter.MutationReplay> edits =
                    WALSplitter.getMutationsFromWALEntry(
                        entry,
                        cells,
                        walEntry,
                        durability
                ...
                if (edits != null && !edits.isEmpty()) {
                    ...
                    Collections.sort(edits, (v1, v2) ->
                        Row.COMPARATOR.compare(v1.mutation, v2.mutation)
                    );
                    long replaySeqId = (entry.getKey().hasOrigSequenceNumber())
                        ? entry.getKey().getOrigSequenceNumber()
                        : entry.getKey().getLogSequenceNumber();
                    OperationStatus[] result = doReplayBatchOp(
                        region,
                        edits,
                        replaySeqId
                    );
            ...
            if (coprocessorHost != null) {
                for (Pair<WALKey, WALEdit> entry : walEntries) {
                    coprocessorHost.postWALRestore(
                        region.getRegionInfo(),
                        entry.getFirst(),
                        entry.getSecond()
                    );
    ...
    public ReplicateWALEntryResponse replicateWALEntry(
        ...
        try {
            ...
            if (regionServer.replicationSinkHandler != null) {
                ...
                regionServer.replicationSinkHandler.replicateLogEntries(
                    entries,
                    cellScanner,
                ...
                throw new ServiceException(
                    "Replication services are not initialized yet"
                );
            }
        } catch (IOException ie) {
            throw new ServiceException(ie);
    ...
    public RollWALWriterResponse rollWALWriter(
        final RpcController controller,
        final RollWALWriterRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            requestCount.increment();
            ...
            RollWALWriterResponse.Builder builder =
                RollWALWriterResponse.newBuilder();
            return builder.build();
        } catch (IOException ie) {
            throw new ServiceException(ie);
        }
    ...
    public UpdateFavoredNodesResponse updateFavoredNodes(
        ...
        for (UpdateFavoredNodesRequest.RegionUpdateInfo regionUpdateInfo : openInfoList) {
            RegionInfo hri = ProtobufUtil.toRegionInfo(
                regionUpdateInfo.getRegion()
            );
    ...
    public BulkLoadHFileResponse bulkLoadHFile(
        final RpcController controller,
        final BulkLoadHFileRequest request
    ) throws ServiceException {
        ...
        try {
            ...
            if (QuotaUtil.isQuotaEnabled(getConfiguration())) {
                ...
                if (enforcement != null) {
                    ...
                    enforcement.checkBulkLoad(
                        regionServer.getFileSystem(),
                        filePaths
                    );
                }
            }
            ...
            for (FamilyPath familyPath : request.getFamilyPathList()) {
                familyPaths.add(
                    new Pair<>(
                        familyPath.getFamily().toByteArray(),
                        familyPath.getPath()
                    )
                );
            }
            if (!request.hasBulkToken()) {
                ...
                try {
                    map = region.bulkLoadHFiles(
                        familyPaths,
                        request.getAssignSeqNum(),
                        null,
                        request.getCopyFile()
                    );
                } finally {
                    if (region.getCoprocessorHost() != null) {
                        region
                            .getCoprocessorHost()
                            .postBulkLoadHFile(familyPaths, map);
                    }
                }
            } else {
                // secure bulk load
                map = regionServer.secureBulkLoadManager.secureBulkLoadHFiles(
                    region,
                    request
                );
            }
    ...
    @Override
    public PrepareBulkLoadResponse prepareBulkLoad(
        RpcController controller,
        ...
        try {
            ...
            String bulkToken =
                regionServer.secureBulkLoadManager.prepareBulkLoad(
                    region,
                    request
                );
            PrepareBulkLoadResponse.Builder builder =
                PrepareBulkLoadResponse.newBuilder();
    ...
    @Override
    public CleanupBulkLoadResponse cleanupBulkLoad(
        RpcController controller,
        ...
        try {
            ...
            CleanupBulkLoadResponse response =
                CleanupBulkLoadResponse.newBuilder().build();
            return response;
        } catch (IOException ie) {
            throw new ServiceException(ie);
        }
    ...
    public CoprocessorServiceResponse execService(
        final RpcController controller,
        final CoprocessorServiceRequest request
    ) throws ServiceException {
        try {
            checkOpen();
            requestCount.increment();
            HRegion region = getRegion(request.getRegion());
            com.google.protobuf.Message result = execServiceOnRegion(
                region,
                request.getCall()
            );
            ...
            builder.setRegion(
                RequestConverter.buildRegionSpecifier(
    ...
    private com.google.protobuf.Message execServiceOnRegion(
        HRegion region,
        final ClientProtos.CoprocessorServiceCall serviceCall
    ) throws IOException {
    ...
    public GetResponse get(
        ...
        try {
            ...
            quota = getRpcQuotaManager().checkQuota(
                region,
            ...
            if (existence == null) {
                ...
                if (get.getExistenceOnly()) {
                    ...
                    if (region.getCoprocessorHost() != null) {
                        exists = region
                            .getCoprocessorHost()
                            .postExists(clientGet, exists);
                    }
                    existence = exists;
                }
            }
            if (existence != null) {
                ClientProtos.Result pbr = ProtobufUtil.toResult(
                    existence,
                ...
                if (
                    ...
                    VersionInfoUtil.hasMinimumVersion(
                        context.getClientVersionInfo(),
                        1,
                        3
                    )
                ) {
                    ...
                    ((HBaseRpcController) controller).setCellScanner(
                        CellUtil.createCellScanner(r.rawCells())
                    );
            ...
            if (mrs != null) {
                TableDescriptor td = region != null
    ...
    private Result get(
        ...
        if (region.getCoprocessorHost() != null) {
            if (region.getCoprocessorHost().preGet(get, results)) {
                ...
                return Result.create(
                    results,
                    get.isCheckExistenceOnly() ? !results.isEmpty() : null,
                    stale
                );
            }
        }
        ...
        if (scan.getLoadColumnFamiliesOnDemandValue() == null) {
            scan.setLoadColumnFamiliesOnDemand(
                region.isLoadingCfsOnDemandDefault()
            );
        }
    ...
    private void checkBatchSizeAndLogLargeSize(MultiRequest request) {
        ...
        for (RegionAction regionAction : request.getRegionActionList()) {
            if (sum == 0) {
                firstRegionName = Bytes.toStringBinary(
                    regionAction.getRegion().getValue().toByteArray()
                );
            }
    ...
    public MultiResponse multi(
        ...
        CellScanner cellScanner = controller != null
        ...
        for (RegionAction regionAction : request.getRegionActionList()) {
            ...
            try {
                ...
                regionActionResultBuilder.setException(
                    ResponseConverter.buildException(e)
                );
                responseBuilder.addRegionActionResult(
                    regionActionResultBuilder.build()
                );
            ...
            if (regionAction.hasAtomic() && regionAction.getAtomic()) {
                ...
                try {
                    if (request.hasCondition()) {
                        ...
                        byte[] qualifier = condition
                            .getQualifier()
                            .toByteArray();
                        CompareOperator op = CompareOperator.valueOf(
                            condition.getCompareType().name()
                        );
                        ByteArrayComparable comparator =
                            ProtobufUtil.toComparator(
                                condition.getComparator()
                            );
                        processed = checkAndRowMutate(
                            region,
                            regionAction.getActionList(),
                            cellScanner,
                            row,
                ...
                cellsToReturn = doNonAtomicRegionMutation(
                    region,
                    quota,
                    regionAction,
                    cellScanner,
        ...
        if (
            cellsToReturn != null &&
            !cellsToReturn.isEmpty() &&
            controller != null
        ) {
            controller.setCellScanner(
                CellUtil.createCellScanner(cellsToReturn)
            );
        }
        ...
        MultiRegionLoadStats.Builder builder =
            MultiRegionLoadStats.newBuilder();
        for (Entry<
            RegionSpecifier,
            ClientProtos.RegionLoadStats
        > stat : regionStats.entrySet()) {
    ...
    private void skipCellsForMutations(
        List<Action> actions,
        CellScanner cellScanner
    ) {
    ...
    public MutateResponse mutate(
        ...
        try {
                ...
                case PUT:
                    ...
                    if (request.hasCondition()) {
                        ...
                        byte[] qualifier = condition
                            .getQualifier()
                            .toByteArray();
                        CompareOperator compareOp = CompareOperator.valueOf(
                            condition.getCompareType().name()
                        );
                        ByteArrayComparable comparator =
                            ProtobufUtil.toComparator(
                        ...
                        if (processed == null) {
                            boolean result = region.checkAndMutate(
                                row,
                                family,
                                qualifier,
                                compareOp,
                    ...
                    break;
                case DELETE:
                    Delete delete = ProtobufUtil.toDelete(
                        mutation,
                        cellScanner
                    );
            ...
            boolean clientCellBlockSupported = isClientCellBlockSupport(
                context
            );
    ...
    private RegionScannerHolder getRegionScanner(ScanRequest request)
        throws IOException {
        ...
        if (rsh == null) {
            ...
            if (closedScanners.getIfPresent(scannerName) != null) {
                ...
                LOG.warn(
                    "Client tried to access missing scanner " + scannerName
                );
        ...
        if (regionServer.getOnlineRegion(hri.getRegionName()) != rsh.r) {
            String msg =
                "Region has changed on the scanner " +
                scannerName +
                ": regionName=" +
    ...
    private RegionScannerHolder newRegionScanner(
        ScanRequest request,
        ScanResponse.Builder builder
    ) throws IOException {
        HRegion region = getRegion(request.getRegion());
    ...
    private void checkScanNextCallSeq(
        ScanRequest request,
    ...
    private long getTimeLimit(
        ...
        if (
            allowHeartbeatMessages &&
            (scannerLeaseTimeoutPeriod > 0 || rpcTimeout > 0)
        ) {
            long timeLimitDelta;
            if (scannerLeaseTimeoutPeriod > 0 && rpcTimeout > 0) {
                timeLimitDelta = Math.min(
                    scannerLeaseTimeoutPeriod,
                    rpcTimeout
                );
            } else {
                timeLimitDelta = scannerLeaseTimeoutPeriod > 0
                    ? scannerLeaseTimeoutPeriod
                    : rpcTimeout;
            }
    ...
    private void checkLimitOfRows(
        int numOfCompleteRows,
        int limitOfRows,
        boolean moreRows,
        ...
        if (numOfCompleteRows >= limitOfRows) {
            if (LOG.isTraceEnabled()) {
                LOG.trace(
                    "Done scanning, limit of rows reached, moreRows: " +
                        moreRows +
                        " scannerContext: " +
                        scannerContext
                );
    ...
    private void scan(
        ...
        try {
            ...
            synchronized (scanner) {
                ...
                boolean allowHeartbeatMessages =
                    clientHandlesHeartbeats && allowPartialResults;
                ...
                final LimitScope timeScope = allowHeartbeatMessages
                    ? LimitScope.BETWEEN_CELLS
                    : LimitScope.BETWEEN_ROWS;

                boolean trackMetrics =
                    request.hasTrackScanMetrics() &&
                ...
                while (numOfResults < maxResults) {
                    ...
                    if (!values.isEmpty()) {
                        ...
                        lastBlock.setValue(
                            addSize(context, r, lastBlock.getValue())
                        );
                        results.add(r);
                        numOfResults++;
                    ...
                    boolean sizeLimitReached = scannerContext.checkSizeLimit(
                        LimitScope.BETWEEN_ROWS
                    );
                    boolean timeLimitReached = scannerContext.checkTimeLimit(
                        LimitScope.BETWEEN_ROWS
                    );
                    ...
                    if (limitReached || !moreRows) {
                        ...
                        if (moreRows) {
                            ...
                            if (timeLimitReached && rsh.needCursor) {
                                ...
                                if (cursorCell != null) {
                                    builder.setCursor(
                                        ProtobufUtil.toCursor(cursorCell)
                                    );
                                }
                            }
                        }
                ...
                if (trackMetrics) {
                    Map<String, Long> metrics = scannerContext
                        .getMetrics()
                        .getMetricsMap();
                    ScanMetrics.Builder metricBuilder =
                        ScanMetrics.newBuilder();
                    NameInt64Pair.Builder pairBuilder =
        ...
        if (region.getCoprocessorHost() != null) {
            region
                .getCoprocessorHost()
                .postScannerNext(scanner, results, maxResults, true);
        }
    }
    ...
    public ScanResponse scan(
        ...
        if (controller != null && !(controller instanceof HBaseRpcController)) {
            throw new UnsupportedOperationException(
                "We only do " +
                    "HBaseRpcControllers! FIX IF A PROBLEM: " +
                    controller
            );
        }
        ...
        boolean closeScanner = request.hasCloseScanner()
        ...
        try {
            ...
            if (rows > 0) {
                ...
                if (region.getCoprocessorHost() != null) {
                    Boolean bypass = region
                        .getCoprocessorHost()
                        .preScannerNext(scanner, results, rows);
                    if (!results.isEmpty()) {
            ...
            if (
                builder.getMoreResults() &&
                builder.getMoreResultsInRegion() &&
                !results.isEmpty()
            ) {
            ...
            try {
                ...
                if (
                    ...
                    throw new UnknownScannerException(
                        "Throwing UnknownScannerException to reset the client" +
                            " scanner state for clients older than 1.3.",
                        e
                    );
                }
            } catch (IOException ioe) {
    ...
    public GetSpaceQuotaSnapshotsResponse getSpaceQuotaSnapshots(
        ...
        try {
            ...
            if (manager != null) {
                final Map<TableName, SpaceQuotaSnapshot> snapshots =
                    manager.copyQuotaSnapshots();
                for (Entry<
                    TableName,
                    SpaceQuotaSnapshot
                > snapshot : snapshots.entrySet()) {
                    builder.addSnapshots(
                        TableQuotaSnapshot.newBuilder()
                            .setTableName(
                                ProtobufUtil.toProtoTableName(snapshot.getKey())
                            )
                            .setSnapshot(
    ...
    public ClearRegionBlockCacheResponse clearRegionBlockCache(
        ...
        for (HRegion region : regions) {
            try {
                stats = stats.append(
                    this.regionServer.clearRegionBlockCache(region)
                );
            } catch (Exception e) {
                stats.addException(region.getRegionInfo().getRegionName(), e);
            }
        }
        stats.withMaxCacheSize(
            regionServer.getCacheConfig().getBlockCache().getMaxSize()
        );
        return builder
        ...
        try {
            ...
            if (request.getProcCount() > 0) {
                for (RemoteProcedureRequest req : request.getProcList()) {
                    ...
                    try {
                        callable = Class.forName(req.getProcClass())
                            .asSubclass(RSProcedureCallable.class)
                            .newInstance();
                    } catch (Exception e) {
                        regionServer.remoteProcedureComplete(
                            req.getProcId(),
                            e
                        );
                        continue;
